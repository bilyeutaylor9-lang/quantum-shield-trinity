...
const buildNormalizedFindingsReport = (report = {}, runtimeOptions = {}) => {
  const normalizedFindings = [
    ...
    ...(report.ragSecurityReport?.findings ?? []).map((item) =>
      normalizeFindingShape(item, "ragSecurityEngine")
    ),
...
export function runQuantumShieldScan(targetDirectory = "src", options = {}) {
  ...
  const promptInjectionReport = timedStep("promptInjectionEngine", () =>
  ...
  const ragSecurityReport = timedStep("ragSecurityEngine", () =>
    ragSecurityEngine(scanResult.files)
  );

  const agentMemorySecurityReport = timedStep("agentMemorySecurityEngine", () =>
  ...
  const securityAuditLoopReport = timedStep("securityAuditLoopEngine", () =>
    securityAuditLoopEngine({
      ...
      systemState: {
        ...
        ragSecurityRiskLevel: ragSecurityReport.summary?.riskLevel ?? "UNKNOWN",
        ragSecurityRiskScore: ragSecurityReport.summary?.riskScore ?? null,
        ragSecurityFindings: ragSecurityReport.summary?.totalFindings ?? 0,
        ragMissingValidation: ragSecurityReport.summary?.missingValidation ?? 0,
        agentMemoryRiskLevel: agentMemorySecurityReport.summary?.riskLevel ?? "UNKNOWN",
  ...
  timedStep("evidenceGraph.add.promptInjectionEngine", () =>
    addEvidenceItemsToGraph(
      evidenceGraph,
      limitEvidenceItems(promptInjectionReport.findings, runtimeOptions.maxEvidenceItems),
      "promptInjectionEngine",
      "prompt_injection"
    )
  );
  ...
  timedStep("evidenceGraph.add.ragSecurityEngine", () =>
    addEvidenceItemsToGraph(
      evidenceGraph,
      limitEvidenceItems(ragSecurityReport.findings, runtimeOptions.maxEvidenceItems),
      "ragSecurityEngine",
      "rag_security"
    )
  );

  timedStep("evidenceGraph.add.agentMemorySecurityEngine", () =>
    addEvidenceItemsToGraph(
      evidenceGraph,
      limitEvidenceItems(agentMemorySecurityReport.findings, runtimeOptions.maxEvidenceItems),
      "agentMemorySecurityEngine",
  ...
  timedStep("evidenceGraph.add.aiAgentRiskEngine", () =>
    addEvidenceItemsToGraph(
      evidenceGraph,
      limitEvidenceItems(aiAgentRiskReport.findings, runtimeOptions.maxEvidenceItems),
      "aiAgentRiskEngine",
      "ai_agent_risk"
    )
  );
  ...
  const executiveReportEngineReport = timedStep("executiveReportEngine", () =>
    executiveReportEngine({
      ...
      riskProfile: {
        ...
        ragSecurity: {
          riskLevel: ragSecurityReport.summary?.riskLevel ?? "UNKNOWN",
          riskScore: ragSecurityReport.summary?.riskScore ?? null,
          totalFindings: ragSecurityReport.summary?.totalFindings ?? 0,
          capabilities: ragSecurityReport.summary?.detectedCapabilities ?? [],
          missingValidation: ragSecurityReport.summary?.missingValidation ?? 0
        },
        agentMemorySecurity: {
          riskLevel: agentMemorySecurityReport.summary?.riskLevel ?? "UNKNOWN",
          riskScore: agentMemorySecurityReport.summary?.riskScore ?? null,
        ...
        aiAgentRisk: {
          ...
          totalFindings: aiAgentRiskReport.summary?.totalFindings ?? 0
        },
        normalizedFindings: {
    ...
    jsonExportEngine({
      ...
      riskProfile: {
        ...
        promptInjection: {
          riskLevel: promptInjectionReport.summary?.riskLevel ?? "UNKNOWN",
          riskScore: promptInjectionReport.summary?.riskScore ?? null,
          findings: promptInjectionReport.summary?.totalFindings ?? 0,
          critical: promptInjectionReport.summary?.critical ?? 0,
          high: promptInjectionReport.summary?.high ?? 0,
        ...
        ragSecurity: {
          riskLevel: ragSecurityReport.summary?.riskLevel ?? "UNKNOWN",
          riskScore: ragSecurityReport.summary?.riskScore ?? null,
          findings: ragSecurityReport.summary?.totalFindings ?? 0,
          critical: ragSecurityReport.summary?.critical ?? 0,
          high: ragSecurityReport.summary?.high ?? 0,
          medium: ragSecurityReport.summary?.medium ?? 0,
          low: ragSecurityReport.summary?.low ?? 0,
          capabilities: ragSecurityReport.summary?.detectedCapabilities ?? [],
          missingValidation: ragSecurityReport.summary?.missingValidation ?? 0
        },
        agentMemorySecurity: {
          riskLevel: agentMemorySecurityReport.summary?.riskLevel ?? "UNKNOWN",
          riskScore: agentMemorySecurityReport.summary?.riskScore ?? null,
          findings: agentMemorySecurityReport.summary?.totalFindings ?? 0,
          critical: agentMemorySecurityReport.summary?.critical ?? 0,
          high: agentMemorySecurityReport.summary?.high ?? 0,
          medium: agentMemorySecurityReport.summary?.medium ?? 0,
          low: agentMemorySecurityReport.summary?.low ?? 0,
          capabilities: agentMemorySecurityReport.summary?.detectedCapabilities ?? [],
          missingControls: agentMemorySecurityReport.summary?.missingControls ?? 0
        },
        aiAgentRisk: {
          riskLevel: aiAgentRiskReport.summary?.riskLevel ?? "UNKNOWN",
          riskScore: aiAgentRiskReport.summary?.riskScore ?? null,
          findings: aiAgentRiskReport.summary?.totalFindings ?? 0,
          critical: aiAgentRiskReport.summary?.critical ?? 0,
          high: aiAgentRiskReport.summary?.high ?? 0,
          medium: aiAgentRiskReport.summary?.medium ?? 0,
          low: aiAgentRiskReport.summary?.low ?? 0
        },
        deepScan: {
          codeFlowRiskLevel: codeFlowReport.riskLevel,
          routeExposureRiskLevel: routeExposureReport.routeExposureRiskLevel,
  ...
  console.log("");
  console.log("[QST] Prompt Injection AI Security");
  console.log("----------------------------------");
  ...
  console.log("");
  console.log("[QST] RAG Security");
  console.log("------------------");
  console.log(`[QST] Risk Score: ${ragSecurityReport.summary?.riskScore ?? "N/A"}`);
  console.log(`[QST] Risk Level: ${ragSecurityReport.summary?.riskLevel ?? "UNKNOWN"}`);
  console.log(`[QST] Findings: ${ragSecurityReport.summary?.totalFindings ?? 0}`);
  console.log(`[QST] RAG Capabilities: ${(ragSecurityReport.summary?.detectedCapabilities ?? []).join(", ") || "None"}`);
  console.log(`[QST] Missing Validation: ${ragSecurityReport.summary?.missingValidation ?? 0}`);

  console.log("");
  console.log("[QST] Agent Memory Security");
  console.log("---------------------------");
  console.log(`[QST] Risk Score: ${agentMemorySecurityReport.summary?.riskScore ?? "N/A"}`);
  console.log(`[QST] Risk Level: ${agentMemorySecurityReport.summary?.riskLevel ?? "UNKNOWN"}`);
  ...
  console.log("-------------------");
  console.log(`[QST] Risk Score: ${aiAgentRiskReport.summary?.riskScore ?? "N/A"}`);
  console.log(`[QST] Risk Level: ${aiAgentRiskReport.summary?.riskLevel ?? "UNKNOWN"}`);
  console.log(`[QST] Findings: ${aiAgentRiskReport.summary?.totalFindings ?? 0}`);
